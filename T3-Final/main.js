...
import { keyboardUpdateMobile } from './moveTanksMobile.js';
...
// ATRASANDO A FUNÇÃO RENDER EM 5 SEGUNDOS
setTimeout(() => {
    render()
}, 3000);
...
window.addEventListener('wheel', (event) => {
    if (event.deltaY > 0) {
        handleZoom(camera, true,orbit);
    } else {
        handleZoom(camera, false,orbit);
    }
});
...
function render(time)
{
        ...
        keyboardUpdate(keyboard, playerTank, scene, projectilesArray, healthPlayer, initialLife)

        // GODMODE
        //godMode(playerTank, keyboard, healthPlayer, initialLife)
        ...
        // TECLADO MOBILE
        if (isMobile) {
            keyboardUpdateMobile(playerTank, scene, projectilesArray);
        }
        ...
        // CAMERA
        updateCamera(camera, playerTank)       
}
}
